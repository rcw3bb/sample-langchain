"""
ReAct agent module using MCP (Model Context Protocol) with ChatOpenAI and GitHub Models.

This module demonstrates the use of MCP (Model Context Protocol) for providing tools
to LangChain agents, as an alternative to the standard LangChain tool decorators.

Key differences from simple_agent_react_chatopenai_tool.py:
...
async def main() -> None:
    """
    Main function to run the MCP-based ReAct agent.
    """
    load_dotenv()
    ...
    print("Using create_react_agent with ChatOpenAI and MCP tools...")
